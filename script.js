...
// Показывает любую ошибку прямо на экране игры красной плашкой сверху.
// Раньше при ошибке всё просто "молча" переставало работать — теперь
// видно, что именно сломалось, без консоли разработчика.
window.addEventListener("error", function(e){
  try{
    const host = document.getElementById("app") || document.body;
    if(!host || host.querySelector(".fatal-error-banner")) return;
    const banner = document.createElement("div");
    banner.className = "fatal-error-banner";
    banner.style.cssText = "background:#7A1030;color:#fff;padding:10px 14px;font-size:12px;line-height:1.5;text-align:center;position:relative;z-index:999;";
    banner.textContent = "Ошибка в игре: " + (e.message || e);
    host.prepend(banner);
  }catch(_){}
});
...
function renderBalance(){
  ...
  const tapEl = $("#perTapValue");
  if(tapEl) tapEl.textContent = "+" + fmt(clickPower());
  const secEl = $("#perSecValue");
  if(secEl) secEl.textContent = "+" + fmt(autoIncomePerSec());
...
function maybeSpawnMemePopup(){
  ...
  if(!floatLayer || !cfg.enabled || !cfg.images.length) return;
...
if(memeBoostBtn) memeBoostBtn.addEventListener("click", activateMemeBoost);
...
function renderWheel(){
  const wheelEl = $("#wheel");
  if(!wheelEl) return; // старая версия index.html без вкладки "Колесо" — просто пропускаем
  ...
  const costLabel = $("#wheelCostLabel");
  if(costLabel) costLabel.textContent = fmt(CONFIG.wheel.costPerSpin);
  updateSpinButton();
}
...
function updateSpinButton(){
  ...
  const label = $("#spinBtnLabel");
  if(label) label.textContent = isSpinning ? "Крутится…" : `Крутить за ${fmt(CONFIG.wheel.costPerSpin)}`;
}
...
function init(){
  try{
    ...
    console.error("MellStroy init error:", e);
  }

  // сохраняем прогресс перед закрытием/сворачиванием — вне try, чтобы точно навесилось
