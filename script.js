...
const CONFIG = {
  ...
  wheelOfFortune: {
    spinCost: 100000,
    // сумма весов обычных секторов = 100, поэтому вес ≈ шанс в процентах.
    // Джекпоты (special:true) выделяются золотым цветом на колесе.
    // Вес 5М подобран так, чтобы после нормализации шанс был ≈1%.
    segments: [
      ...
      { label: "1М",   value: 1000000, weight: 3,     special:true },
      { label: "2М",   value: 2000000, weight: 1.5,   special:true },
      { label: "5М🔥", value: 5000000, weight: 1.06,  special:true }
    ]
  }
};
...
function fmt(n){
  ...
  if(n < 1_000_000)     return trimNum(n / 1_000)       + " тыс";
  if(n < 1_000_000_000) return trimNum(n / 1_000_000)   + " млн";
  return trimNum(n / 1_000_000_000) + " млрд";
}

// округляет до 2 знаков после запятой, убирает лишние нули (5.00 -> 5, 1.50 -> 1,5)
// и ставит запятую вместо точки, как принято в русской локали
function trimNum(x){
