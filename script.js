...
const CONFIG = {
  ...
  baseClickPower: 1,          // сколько MELLcoin за тап на уровне 1 "Силы клика"
  ...
  upgrades: {
    ...
    autoClicker: {
      ...
      baseCost: 100,
      costGrowth: 1.6,
      valuePerLevel: 1,      // +1 MELLcoin/сек за уровень
      maxLevel: 50
    },
    memeBoostPower: {
      name: "Мем-буст+",
  ...
  achievements: [
    ...
    { id: "novice", name: "Новичок", desc: "Заработай 1 000 MELLcoin", icon: "🌱", reward: 100,
      check: s => s.totalEarned >= 1000 },
    { id: "meme_mogul", name: "Мемный магнат", desc: "Заработай 100 000 MELLcoin", icon: "💰", reward: 5000,
  ...
  rareEvents: {
    ...
    messages: [
      { icon: "🎲", title: "Редкий дроп!", text: "MellStroy кинул тебе бонус: +500 MELLcoin", bonus: 500 },
      { icon: "📦", title: "Мелстрой кейс", text: "Внутри оказалось +250 MELLcoin", bonus: 250 },
      { icon: "🚀", title: "Ты на высотах", text: "Донат-эффект принёс +1500 MELLcoin", bonus: 1500 },
      { icon: "🐸", title: "Пепе услышал тебя", text: "+300 MELLcoin просто так", bonus: 300 }
    ]
  },
  ...
  // ---- колесо фортуны ----
  // spinCost — цена одного прокрута в MELLcoin.
  ...
  wheelOfFortune: {
    spinCost: 100000,
    // сумма весов секторов ≤100К = 98 (почти не трогали), у секторов >100К
    // веса подняты, поэтому после нормализации их шанс выпадения выше, чем был.
    segments: [
      ...
      { label: "200К", value: 200000,  weight: 3,    special:true },
      { label: "500К", value: 500000,  weight: 1.5,  special:true },
      { label: "1М",   value: 1000000,  weight: 5,   special:true },
      { label: "2М",   value: 2000000,  weight: 3,   special:true },
      { label: "5М",   value: 5000000,  weight: 1.3, special:true },
      { label: "10М",  value: 10000000, weight: 0.3, special:true }
    ]
  },

  // ---- колесо ставок (множители) ----
  // игрок сам вводит сумму ставки, колесо даёт множитель к ней.
  // x5 намеренно редкий (маленький weight), x0 и x0.5 — самые частые.
  betWheel: {
    minBet: 1000,
    segments: [
      { label: "x0",   mult: 0,   weight: 30 },
      { label: "x0.5", mult: 0.5, weight: 25 },
      { label: "x1",   mult: 1,   weight: 20 },
      { label: "x2",   mult: 2,   weight: 15 },
      { label: "x3",   mult: 3,   weight: 7,  special:true },
      { label: "x5",   mult: 5,   weight: 3,  special:true }
    ]
  }
};
...
function defaultState(){
  return {
    ...
    promoActive: false,
    promoDepActive: false,
    thanksForPlayingRedeemed: false,
    _tapSinceRare: 0,
    _saveVersion: 1
  };
}
...
function fmt(n){
  ...
  if(n < 1_000_000)     return trimDec(n / 1_000, 1)       + "k";
  if(n < 1_000_000_000) return trimDec(n / 1_000_000, 2)   + "m";
  return trimDec(n / 1_000_000_000, 2) + "b";
}

// округляет x до decimals знаков и убирает лишний хвост нулей
// (5.00 -> "5", 7.10 -> "7,1"), запятая вместо точки — под русскую локаль
function trimDec(x, decimals){
...
function clickPower(){
  // промокод YURAGEIOBOSANI даёт этому игроку фиксированные 100k за тап,
  // независимо от купленных улучшений
  if(state.promoActive) return 100000;

  // промокод YURADEP даёт этому игроку фиксированные 20k за тап,
  // независимо от купленных улучшений (YURADEPoff — отменяет)
  if(state.promoDepActive) return 20000;
...
// прогресс "уровня игрока" на главном экране — использует ту же формулу,
// что и уровень в профиле (1 уровень = каждые 5000 заработанных MELLcoin)
const LEVEL_STEP = 5000;
function renderLevelStrip(){
  const level = 1 + Math.floor(state.totalEarned / LEVEL_STEP);
  const intoLevel = state.totalEarned % LEVEL_STEP;
  const remaining = LEVEL_STEP - intoLevel;
  const pct = Math.min(100, (intoLevel / LEVEL_STEP) * 100);

  $("#levelStripNum").textContent = level;
  $("#levelStripNext").textContent = `до след. уровня: ${fmt(remaining)}`;
  $("#levelStripFill").style.width = pct + "%";
}
...
const PROMO_CODE_ON     = "YURAGEIOBOSANI";
const PROMO_CODE_OFF    = "YURAGEIOBOSANIoff";
const PROMO_CODE_THANKS = "THANKSFORPLAYING";
const PROMO_THANKS_REWARD = 500000;
const PROMO_CODE_DEP     = "YURADEP";
const PROMO_CODE_DEP_OFF = "YURADEPoff";
const PROMO_CODE_ADMIN   = "ADMIN";

const promoBtn = $("#promoBtn");
const promoInput = $("#promoInput");
const navbarEl = $(".navbar");

// на мобильных клавиатура при фокусе на input перестраивает разметку —
// первый тап по кнопке рядом просто закрывает клавиатуру и промахивается
// мимо. pointerdown-заглушка перед click убирает эту гонку.
promoBtn.addEventListener("pointerdown", (e)=> e.preventDefault());
promoBtn.addEventListener("click", redeemPromoCode);
promoInput.addEventListener("keydown", (e)=>{
  if(e.key === "Enter") redeemPromoCode();
});

// фикс "промокод залезает на нижнее меню": fixed-navbar не двигается вместе
// с visual viewport, когда открыта клавиатура, и визуально наезжает на
// промо-блок. Прячем navbar на время, пока инпут в фокусе.
promoInput.addEventListener("focus", ()=> navbarEl?.classList.add("navbar--hidden"));
promoInput.addEventListener("blur",  ()=> navbarEl?.classList.remove("navbar--hidden"));
...
function redeemPromoCode(){
  ...
  if(code === PROMO_CODE_ON){
    if(state.promoActive){
      showPromoMsg("Промокод уже активирован для тебя ✅", "ok");
    } else {
      state.promoActive = true;
      saveState();
      renderBalance();
      showPromoMsg("Промокод активирован! Теперь у тебя +100k за тап и повышенные шансы в колесе фортуны 🔥", "ok");
    }
  } else if(code === PROMO_CODE_OFF){
    if(!state.promoActive){
      showPromoMsg("Промокод и так не был активен", "err");
    } else {
      state.promoActive = false;
      saveState();
      renderBalance();
      showPromoMsg("Промокод деактивирован — всё вернулось как было", "ok");
    }
  } else if(code === PROMO_CODE_DEP){
    if(state.promoDepActive){
      showPromoMsg("Промокод уже активирован для тебя ✅", "ok");
    } else {
      state.promoDepActive = true;
      saveState();
      renderBalance();
      showPromoMsg("Промокод активирован! Теперь у тебя +20k за тап 🔥", "ok");
    }
  } else if(code === PROMO_CODE_DEP_OFF){
    if(!state.promoDepActive){
      showPromoMsg("Промокод и так не был активен", "err");
    } else {
      state.promoDepActive = false;
      saveState();
      renderBalance();
      showPromoMsg("Промокод деактивирован — всё вернулось как было", "ok");
    }
  } else if(code === PROMO_CODE_ADMIN){
    showAdminOverlay();
    showPromoMsg("Промокод активирован ✅", "ok");
  } else if(code === PROMO_CODE_THANKS){
    if(state.thanksForPlayingRedeemed){
      showPromoMsg("Этот промокод уже был использован — доступен только 1 раз", "err");
    } else {
      state.thanksForPlayingRedeemed = true;
      state.balance += PROMO_THANKS_REWARD;
      state.totalEarned += PROMO_THANKS_REWARD;
      saveState();
      renderBalance();
      showPromoMsg(`Промокод активирован! +${fmt(PROMO_THANKS_REWARD)} MELLcoin 🎉`, "ok");
    }
  } else {
    showPromoMsg("Неверный промокод", "err");
  }
...
function showPromoMsg(text, kind){
  ...
  if(!el) return;
  el.textContent = text;
  el.classList.remove("promo-card__msg--ok", "promo-card__msg--err");
  el.classList.add(kind === "ok" ? "promo-card__msg--ok" : "promo-card__msg--err");
}

// картинка на весь экран (assets/admin.png), пока играет
// assets/admin-music.mp3 — прячется сама, как только трек доигрывает до конца
function showAdminOverlay(){
  const overlay = $("#adminOverlay");
  const audio = $("#sfxAdmin");
  if(!overlay || !audio) return;

  overlay.classList.add("active");

  audio.currentTime = 0;
  audio.onended = () => overlay.classList.remove("active");
  audio.play().catch(()=>{
    // автоплей мог быть заблокирован браузером — прячем картинку сразу,
    // чтобы она не зависла на весь экран без звука
...
function buildWheelSVG(segments){
  ...
  let defs = `<defs>
    <linearGradient id="jackpotFill" x1="0" y1="0" x2="1" y2="1">
  ...
  segments.forEach((seg, i) => {
    ...
    const fill = seg.special ? "url(#jackpotFill)" : bgColors[i % 2];
    const strokeColor = seg.special ? "#FFD23D" : "#362757";
    const strokeW = seg.special ? 2.5 : 2;

    inner += `<path d="M${cx},${cy} L${x1.toFixed(2)},${y1.toFixed(2)} A${R},${R} 0 ${largeArc} 1 ${x2.toFixed(2)},${y2.toFixed(2)} Z" fill="${fill}" stroke="${strokeColor}" stroke-width="${strokeW}"/>`;
    ...
    const textFill = seg.special ? "#FFE8A3" : "#FFD23D";
    const fontSize = seg.special ? 16 : 15;
    inner += `<text x="${lx.toFixed(2)}" y="${ly.toFixed(2)}" transform="rotate(${rot.toFixed(2)} ${lx.toFixed(2)} ${ly.toFixed(2)})" text-anchor="middle" dominant-baseline="middle" fill="${textFill}" font-family="'Rubik Mono One', monospace" font-size="${fontSize}">${escapeHtml(seg.label)}</text>`;
  });
  ...
  return `<svg viewBox="0 0 300 300" width="100%" height="100%" style="display:block;">${defs}${inner}</svg>`;
}
...
// при активном промокоде YURAGEIOBOSANI шансы в колесе смещаются в пользу
// крупных призов: мелкие суммы становятся редкими, крупные — заметно чаще
function segmentWeight(seg){
  const base = seg.weight || 1;
  if(!state.promoActive) return base;
  if(seg.value >= 1_000_000) return base * 25;
  if(seg.value >= 100_000)   return base * 6;
  return base * 0.15;
}
...
function spinWheel(){
  ...
  if(state.balance < cost){
    showModal({
      emoji: "🎡", title: "Недостаточно MELLcoin",
...
/* =====================================================================
   BET WHEEL (колесо ставок — множители x0…x5 на свою сумму)
   ===================================================================== */

let betWheelRotation = 0;
let betWheelSpinning  = false;

function initBetWheel(){
  const wheelEl = $("#wheelBet");
  if(!wheelEl) return;
  wheelEl.innerHTML = buildWheelSVG(CONFIG.betWheel.segments);

  $("#wheelBetSpinBtn").addEventListener("click", spinBetWheel);

  const betInput = $("#betInput");
  betInput.addEventListener("input", renderBetWheelButton);

  $$(".bet-quick__btn").forEach(btn=>{
    btn.addEventListener("click", ()=>{
      const current = Math.max(0, Math.floor(Number(betInput.value) || 0));
      let next;
      if(btn.dataset.bet === "half")   next = Math.floor(current / 2);
      if(btn.dataset.bet === "double") next = current * 2 || CONFIG.betWheel.minBet;
      if(btn.dataset.bet === "max")    next = state.balance;
      next = Math.min(Math.max(next, 0), state.balance);
      betInput.value = next;
      renderBetWheelButton();
    });
  });
...
function renderBetWheelButton(){
  const btn = $("#wheelBetSpinBtn");
  if(!btn || betWheelSpinning) return;
  const bet = Math.floor(Number($("#betInput").value) || 0);
  const valid = bet >= CONFIG.betWheel.minBet && bet <= state.balance;
  btn.disabled = !valid;
  btn.textContent = bet > 0
    ? `🎡 Крутить — ${fmt(bet)} ${CONFIG.currencyName}`
    : "🎡 Крутить ставку";
}

function pickWeightedIndexPlain(segments){
...
function spinBetWheel(){
  if(betWheelSpinning) return;

  const betInput = $("#betInput");
  const bet = Math.floor(Number(betInput.value) || 0);
  const minBet = CONFIG.betWheel.minBet;

  if(bet < minBet){
    showModal({
      emoji: "🎡", title: "Слишком маленькая ставка",
      text: `Минимальная ставка — ${fmt(minBet)} ${CONFIG.currencyName}.`,
      btnText: "Ок"
    });
    return;
  }
  if(bet > state.balance){
    showModal({
      emoji: "🎡", title: "Недостаточно MELLcoin",
      text: `На балансе ${fmt(state.balance)} ${CONFIG.currencyName}, а ставка — ${fmt(bet)}.`,
      btnText: "Ок"
    });
    return;
  }
  ...
  betWheelSpinning = true;
  betInput.disabled = true;
  $$(".bet-quick__btn").forEach(b => b.disabled = true);
  $("#wheelBetSpinBtn").disabled = true;
  $("#wheelBetSpinBtn").textContent = "🎡 Крутится…";

  const segments = CONFIG.betWheel.segments;
  const n = segments.length;
  const anglePer = 360 / n;
  const winIndex = pickWeightedIndexPlain(segments);

  const segCenterFromTop = winIndex * anglePer + anglePer / 2;
  const targetMod = (360 - segCenterFromTop) % 360;
  const currentMod = ((betWheelRotation % 360) + 360) % 360;
  const delta = ((targetMod - currentMod) + 360) % 360;
  const extraSpins = 5;

  betWheelRotation += extraSpins * 360 + delta;

  const wheelEl = $("#wheelBet");
  wheelEl.style.transform = `rotate(${betWheelRotation}deg)`;

  setTimeout(() => {
    const seg = segments[winIndex];
    const payout = Math.round(bet * seg.mult);
    state.balance += payout;
    if(payout > bet) state.totalEarned += (payout - bet);
    saveState();
    renderBalance();
    checkAchievements();
    if(payout > 0) playSound("coin");

    betWheelSpinning = false;
    betInput.disabled = false;
    $$(".bet-quick__btn").forEach(b => b.disabled = false);
    renderBetWheelButton();

    const emoji = seg.mult >= 3 ? "🎉" : seg.mult > 1 ? "🔥" : seg.mult > 0 ? "😐" : "💀";
    const text = payout > 0
      ? `Выпало ${seg.label} — забери ${fmt(payout)} ${CONFIG.currencyName}!`
      : `Выпало ${seg.label} — ставка сгорела. Повезёт в следующий раз!`;

    showModal({ emoji, title: "Колесо ставок", text, btnText: "Ок" });
  }, 4600);
}
...
$("#toBetWheelBtn")?.addEventListener("click", ()=> switchScreen("wheel-bet"));
$("#toMainWheelBtn")?.addEventListener("click", ()=> switchScreen("wheel"));

function switchScreen(name){
  $$(".screen").forEach(s => s.classList.toggle("active", s.dataset.screen === name));
  // колесо ставок не имеет своей кнопки в нижнем меню — держим подсвеченной
  // ту же вкладку "Колесо", что и для основного колеса фортуны
  const navTarget = name === "wheel-bet" ? "wheel" : name;
  $$(".navbtn").forEach(b => b.classList.toggle("active", b.dataset.target === navTarget));
  if(name === "leaders"){ renderLeaderboard(); syncPlayerScore(); }
  if(name === "profile") renderProfile();
  if(name === "upgrades") renderUpgrades();
  if(name === "wheel-bet") renderBetWheelButton();
}
