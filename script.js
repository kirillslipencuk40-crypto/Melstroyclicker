...
function renderPromoStatus(){
  const badge = $("#promoStatusBadge");
  const foot = badge ? badge.closest(".promo-card__foot") : null;
  if(!badge) return;
  badge.textContent = state.promoActive ? "активен ✅" : "не активен";
  if(foot) foot.classList.toggle("active", state.promoActive);
}
